...
これで `add`、`multiply`、`subtract`、`square` 関数をエクスポートできるようになりました。しかし、モジュールから値をエクスポートしただけでは、任意のファイルからそれらの値を使用することはできません。モジュールからエクスポートされた値を使えるようにするためには、値を参照するファイルにおいて明示的に*インポート*する必要があります。
