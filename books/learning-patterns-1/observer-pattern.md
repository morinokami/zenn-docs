...
オブザーバーパターンを使うことは、**関心の分離**と単一責任の原則を実現するための素晴らしい方法です。Observer オブジェクトは Observable オブジェクトと密結合しておらず、いつでも結合 (あるいは疎結合化) することができます。Observable オブジェクトはイベントの監視に責任をもち、Observer は受け取ったデータを処理するだけとなります。
