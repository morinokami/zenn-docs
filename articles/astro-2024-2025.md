...
以上からわかるように、新たに追加された Content Layer は、既存の Content Collections を拡張することにより Astro におけるコンテンツの扱いを一般化し、さらにデータローダーを自由に定義し配布可能とすることでコンテンツの使用可能性をより高めました。また、カスタムローダーを作成する際に使用する [Content Loader API](https://docs.astro.build/en/reference/content-loader-reference/) には、データの更新タイミングの記録などに使用可能な [`meta`](https://docs.astro.build/en/reference/content-loader-reference/#meta) ストアや、データのレンダリング結果を格納するための [`rendered`](https://docs.astro.build/en/reference/content-loader-reference/#rendered) フィールドなど、便利な機能が様々用意されています。Content Layer API のもとで刷新された Content Collections を使い、ぜひ自身のローダーを定義し、出来のいいものは npm パッケージとして公開していきましょう。
...
また、下でも触れる Astro Together において、Container API を駆使し PHP プロジェクト内に Astro コンポーネントをアイランド込みで埋め込むというデモがおこなわれています。こちらも興味がある方はぜひご覧ください:
